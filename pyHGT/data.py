...
class Graph():
    ...
    def get_types(self):
        return sorted(list(self.node_feature.keys()))  # sorted 保证每次返回的顺序一致，类型的编号也就固定了


def sample_subgraph(graph, time_range, sampled_depth = 2, sampled_number = 8, inp = None, feature_extractor = feature_CM):
    '''
    ...
    '''
    feature, times, indxs, ylabels = feature_extractor(layer_data, graph)
    ...
    return feature, times, edge_list, indxs, ylabels


def train_subgraph(graph, time_range, inp, feature_extractor = feature_CM):
    '''
        提取训练集部分的节点特征和边连接
    '''
    layer_data  = defaultdict( #target_type
    ...
    '''
        ...
        link exist in the original graph
    '''
    # 将选中的节点（layer_data）,之间的连接都选出来。
    for target_type in graph.edge_list:
        te = graph.edge_list[target_type]
        tld = layer_data[target_type]  # tld 表示 target_id:[index, time]
        for source_type in te:
            tes = te[source_type]
            sld = layer_data[source_type]
            for relation_type in tes:
                tesr = tes[relation_type]  # tesr 表示原图中的 target_id:[source, time]
                for target_key in tld:
                    if target_key not in tesr:  # 如果原图中的目标节点没被选中，也就是不在tld里面，跳转。
                        continue
                    target_ser = tld[target_key][0]
                    for source_key in tesr[target_key]:
                        '''
                            Check whether each link (target_id, source_id) exist in original adjacancy matrix
                        '''
                        if source_key in sld:  # 如果原图中的源节点没被选中，也就是不在sld里面，跳转。
                            source_ser = sld[source_key][0]
                            edge_list[target_type][source_type][relation_type] += [[target_ser, source_ser]]
        ...
        for source_type in edge_list[target_type]:
            for relation_type in edge_list[target_type][source_type]:
                for ii, (ti, si) in enumerate(edge_list[target_type][source_type][relation_type]):
                    ...
                    '''
                    # edge_time  += [int(node_time[tid]) - int(node_time[sid]) + 2]
                    edge_time += [0]

    # node_feature = torch.FloatTensor(node_feature)
    for t in types:
        n_feature = torch.FloatTensor(feature[t])
        node_feature.append(n_feature)
