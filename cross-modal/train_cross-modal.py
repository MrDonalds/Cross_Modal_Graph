...
def node_classification_sample(seed, pairs, time_range):
    ...
    '''
    np.random.seed(seed)
    target_ids = np.random.choice(list(pairs.keys()), args.batch_size//len(types), replace=False)
    target_info = []
...
def node_classification(seed, pairs, time_range):
...
#         p = pool.apply_async(node_classification_sample, args=(randint(), train_pairs, train_range))
#         jobs.append(p)
#     p = pool.apply_async(node_classification_sample, args=(randint(), valid_pairs, valid_range))
#     jobs.append(p)
#     return jobs
...
train_data = [node_classification(randint(), train_pairs, [])]
...
for epoch in np.arange(args.n_epoch) + 1:
    ...
    # train_data = [node_classification_sample(randint(), train_pairs, train_range) for batch_id in np.arange(args.n_batch)]
    # valid_data = node_classification_sample(randint(), valid_pairs, valid_range)
    ...
    model.eval()
    with torch.no_grad():
        valid_data = node_classification(randint(), {**train_pairs, **test_pairs}, [])
        ...
        loss = calc_loss(f1, f2, r1, r2, y[node_time == t_v_t], y[node_time == t_v_t], alpha, beta)
...
'''
# test_data = node_classification(randint(), {**train_pairs, **test_pairs}, [])
#
