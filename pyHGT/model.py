...
class GNN(nn.Module):
    def __init__(self, in_dim, n_hid, num_types, num_relations, n_heads, n_layers, dropout = 0.2, conv_name = 'hgt', prev_norm = False, last_norm = False, use_RTE = True):
        ...
        for type_id in range(num_types):
            self.adapt_ws.append(nn.Linear(in_dim[type_id], n_hid))
    ...
    def forward(self, node_feature, node_type, edge_time, edge_index, edge_type):
        ...
        res_size = 0
        for t in node_feature:
            res_size += t.size(0)

        res = torch.zeros(res_size, self.n_hid).to(node_type.device)
        for type_id in range(self.num_types):
            idx = (node_type == int(type_id))
            if idx.sum() == 0:
                continue
            res[idx] = torch.tanh(self.adapt_ws[type_id](node_feature[type_id].to(node_type.device)))
